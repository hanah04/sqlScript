/* 
   Insert item discount for newly added discount scheme
   
   Note:
    @PK_mscDiscounts is the new discount code added (check in mscDiscounts table)
*/
	...
	case
		...
		from iwWareitem a
			...
			cross join mscDiscounts b
				where b.PK_mscDiscounts = @PK_mscDiscounts
					and a.isTrade = 1
					and c.isactive = 1 
					...
					not in (select cast(a.FK_mscWarehouse as varchar) + a.FK_iwItems + cast(a.FK_mscDiscounts as varchar) 
						from iwItemPriceDiscounts a 
							where FK_mscDiscounts = @PK_mscDiscounts)